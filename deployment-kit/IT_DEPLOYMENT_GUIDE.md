...
4. Each invited user gets a personal set-your-password link (valid for 7 days). The invitation email delivers it automatically when email is configured (Resend API key); otherwise, the Admin Panel shows the link after the invite so you can share it manually. Resending an invitation issues a fresh link.
