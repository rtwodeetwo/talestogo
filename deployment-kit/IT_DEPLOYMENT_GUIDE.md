...
ARM deployments are still possible: clone the repository and build from source with `docker compose up -d --build`, which produces an image for whatever architecture you build on. If you need a published ARM image, open an issue at https://github.com/rtwodeetwo/talestogo and one can be added.
...
4. How the user signs in for the first time depends on `ENABLE_LOCAL_AUTH`. With email/password login enabled, each invited user gets a personal set-your-password link (valid for 7 days), delivered by the invitation email when email is configured (Resend API key); otherwise the Admin Panel shows the link after the invite so you can share it manually, and resending an invitation issues a fresh one. On a single-sign-on-only deployment (`ENABLE_LOCAL_AUTH=false`) no link is needed or generated: the account is already approved, and the user simply signs in with Google or Microsoft using the invited email address.
