...
- POST /highlights/quarterly-check: aggregates the previous complete quarter's
  response data and emails a quarterly highlights summary. Alerts instead of
  emailing when the quarter contains no response data.

There is no monthly equivalent: monthly highlights emails were removed.
...
from app.models import BrandInfo, Response, BatchAnalytics, Query
...
def _window_has_responses(db: Session, brand, start_date, end_date) -> bool:
    """Whether any response was collected in the window.

    Replaces the old "was a report generated for this period?" check. A period
    with no collection is the thing worth alerting on; a missing report never
    was, now that reports are not generated at all.
    """
    return db.query(Response.id).filter(
        Response.user_id == brand.user_id,
        Response.brand_id == brand.id,
        Response.timestamp >= start_date,
        # Half-open [start, end): period_ranges now returns an exclusive end.
